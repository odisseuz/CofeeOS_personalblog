...
function loadNote(file, body, filenameEl, statusEl) {
...
function saveNotes() {
  const area = document.getElementById('notes-area');
  if (!area || !currentNoteFile) return;
  try {
    localStorage.setItem('coffeeos:notes:' + currentNoteFile, area.value);
  } catch (e) {}
}
...
function downloadNotes() {
  ...
  const name = (currentNoteFile.split('/').pop() || 'notes').replace(/\.md$/, '') + '.notes.md';
