// busca + card "recent"
import { getAllPosts, getPostIndex, visiveis } from './data.js';
...
export function loadRecentPosts() {
  ...
  getPostIndex().then(function (posts) {
    const sorted = visiveis(posts).sort(byNewest);
  ...
  input.addEventListener('input', function () {
    ...
    timer = setTimeout(function () {
      ...
      getPostIndex().then(function (index) {
        const byTitle = visiveis(index).filter(function (p) {
        ...
        return getAllPosts().then(function (posts) {
          const matches = visiveis(posts).filter(function (p) {
